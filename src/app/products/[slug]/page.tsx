...
export default function ProductDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const displayPrice = useMemo(() => {
    ...
    if (product.name === "Petrol Fuel Injector Cleaning Service" && selectedOption1) {
      return injectorCleaningPricing[selectedOption1] || product.price;
    }
  ...
  const serviceSchema = product.category === "Servicing" ? {
    ...
    "offers": {
      "@type": "Offer",
      "price": displayPrice.replace(/[€,]/g, '').split(' ')[0].trim() || "120",
      "priceCurrency": "EUR"
    }
  } : null;
