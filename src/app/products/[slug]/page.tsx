...
import { ChevronLeft, ChevronRight, ChevronDown, Play, Check, X, ArrowLeft, ShoppingCart, Info, Wrench, Shield, HelpCircle } from "lucide-react";
import { notFound } from "next/navigation";

// ACE Comparison Table Component
function ACEComparisonTable() {
  const features = [
    { feature: "Front Camera Resolution", ace31: "3K (2560x1440)", ace30pro: "2K (2560x1440)", highlight: "31" },
    { feature: "Rear Camera", ace31: "Optional (+€85)", ace30pro: "Included", highlight: "pro" },
    { feature: "Built-in Display", ace31: false, ace30pro: "3.5\" Touchscreen", highlight: "pro" },
    { feature: "Voice Control", ace31: false, ace30pro: true, highlight: "pro" },
    { feature: "Smartphone App", ace31: true, ace30pro: true, highlight: null },
    { feature: "WiFi Connectivity", ace31: true, ace30pro: true, highlight: null },
    { feature: "Bluetooth", ace31: false, ace30pro: true, highlight: "pro" },
    { feature: "GPS Recording", ace31: true, ace30pro: true, highlight: null },
    { feature: "Speed Display on Screen", ace31: false, ace30pro: true, highlight: "pro" },
    { feature: "Parking Mode", ace31: "Motion Activated", ace30pro: "Up to 48 Hours", highlight: "pro" },
    { feature: "G-Sensor (Incident Detection)", ace31: true, ace30pro: true, highlight: null },
    { feature: "Loop Recording", ace31: true, ace30pro: true, highlight: null },
    { feature: "Live View Mode", ace31: "Via App", ace30pro: "On Device + App", highlight: "pro" },
    { feature: "Design", ace31: "Compact/Discreet", ace30pro: "Premium Build", highlight: null },
    { feature: "SD Card Included", ace31: "64GB", ace30pro: "64GB", highlight: null },
    { feature: "Max SD Card Support", ace31: "128GB", ace30pro: "128GB", highlight: null },
    { feature: "Base Price", ace31: "€300", ace30pro: "€500", highlight: "31" },
    { feature: "With Professional Fitting", ace31: "€505", ace30pro: "€780", highlight: "31" },
  ];

  const renderValue = (value: string | boolean, highlight: string | null, column: "31" | "pro") => {
    const isHighlighted = highlight === column || (highlight === "31" && column === "31") || (highlight === "pro" && column === "pro");
  ...
  return (
    <div className="my-8 rounded-2xl overflow-hidden border border-white/10 bg-zinc-900/50">
      ...
      <div className="bg-gradient-to-r from-blue-600/20 via-purple-600/20 to-red-600/20 p-4 border-b border-white/10">
        <h3 className="text-xl font-bold text-white text-center">ACE 3.1 vs ACE 3.0 PRO Comparison</h3>
      </div>
            ...
            <tr className="border-b border-white/10">
              ...
              <th className="text-center p-4 w-1/3">
                <div className="flex flex-col items-center gap-1">
                  <span className="text-blue-400 font-bold">ACE 3.1</span>
                  <span className="text-xs text-gray-500">Latest Gen</span>
                </div>
              </th>
              <th className="text-center p-4 w-1/3 bg-gradient-to-b from-purple-500/10 to-transparent">
                <div className="flex flex-col items-center gap-1">
                  <span className="text-purple-400 font-bold">ACE 3.0 PRO</span>
          ...
          <tbody>
            {features.map((row, index) => (
              ...
              >
                ...
                <td className="p-4 text-center">{renderValue(row.ace31, row.highlight, "31")}</td>
                <td className="p-4 text-center bg-purple-500/5">{renderValue(row.ace30pro, row.highlight, "pro")}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="p-4 bg-black/30 border-t border-white/10">
        <p className="text-xs text-gray-500 text-center">
          All models include 2-year warranty and are genuine BMW/MINI accessories
        </p>
      </div>
    </div>
  );
...
export default function ProductDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Pricing maps for BMW Advanced Car Eye Dashcam
  const aceModelPricing: Record<string, number> = {
    "ACE 3.1 Front Only": 300,
    "ACE 3.1 Front + Rear": 385,
    "ACE 3.0 PRO (Front + Rear)": 500,
  };

  const aceFittingPricing: Record<string, Record<string, number>> = {
    "ACE 3.1 Front Only": {
      "Dashcam Only (No Fitting)": 0,
      "Include Professional Fitting": 205,
    },
    "ACE 3.1 Front + Rear": {
      "Dashcam Only (No Fitting)": 0,
      "Include Professional Fitting": 280,
    },
    "ACE 3.0 PRO (Front + Rear)": {
      "Dashcam Only (No Fitting)": 0,
      "Include Professional Fitting": 280,
    },
  };
  ...
  // Set appropriate image index when model selection changes for Advanced Car Eye
  useEffect(() => {
    if (product.name === "BMW Advanced Car Eye Dashcam" && selectedOption1) {
      // Images array: [ace-3.0.png (index 0), ace-3.1.png (index 1), ace-3.0-fr.png (index 2)]
      if (selectedOption1 === "ACE 3.1 Front Only") {
        setCurrentImageIndex(1); // ace-3.1.png
      } else if (selectedOption1 === "ACE 3.1 Front + Rear" || selectedOption1 === "ACE 3.0 PRO (Front + Rear)") {
        setCurrentImageIndex(2); // ace-3.0-fr.png
      } else {
        setCurrentImageIndex(0); // ace-3.0.png (default)
      }
    }
  }, [selectedOption1, product.name]);
  ...
  const displayPrice = useMemo(() => {
    ...
    if (product.name === "BMW Advanced Car Eye Dashcam") {
      const model = selectedOption1 || "ACE 3.1 Front Only";
      const fitting = selectedOption2 || "Dashcam Only (No Fitting)";
      let total = aceModelPricing[model] || 300;
  ...
  // Labels for ACE thumbnail images
  const getAceImageLabel = (imagePath: string): string | null => {
    if (product.name !== "BMW Advanced Car Eye Dashcam") return null;
  ...
  const getPriceRange = () => {
    ...
    if (product.name === "BMW Advanced Car Eye Dashcam") {
      return { lowPrice: "300", highPrice: "780" };
    }
