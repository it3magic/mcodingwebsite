...
export default function NotificationBannerInline() {
  ...
  useEffect(() => {
    ...
    fetch("/api/notification")
      ...
      .then((data: NotificationData) => {
        ...
        const dismissedTimestamp = sessionStorage.getItem("notification-dismissed-inline");
  ...
  const handleDismiss = () => {
    ...
    if (notification?.lastUpdated) {
      sessionStorage.setItem("notification-dismissed-inline", notification.lastUpdated);
    }
  };
  ...
  return (
    <div className="px-2 sm:px-4 py-2 sm:py-3 animate-slideDown">
